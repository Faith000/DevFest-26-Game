...
export function Header() {
  ...
  return (
    <header className="border-b-2 border-ink bg-paper">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
        <Link href="/" className="flex items-center gap-2">
          <span className="flex gap-1" aria-hidden>
            <span className="h-3 w-3 rounded-full border-2 border-ink bg-core-red" />
            <span className="h-3 w-3 rounded-full border-2 border-ink bg-core-blue" />
            <span className="h-3 w-3 rounded-full border-2 border-ink bg-core-green" />
            <span className="h-3 w-3 rounded-full border-2 border-ink bg-core-amber" />
          </span>
          <span className="font-[family-name:var(--font-grotesk)] text-lg font-bold leading-none">
            Lagos Tech Traffic
        ...
        <button
          className="df-border df-shadow flex h-10 w-10 flex-col items-center justify-center gap-1 bg-white sm:hidden"
          aria-expanded={open}
      ...
      {open && (
        <nav id="mobile-nav" aria-label="Mobile" className="border-t-2 border-ink bg-white sm:hidden">
          <ul>
            {NAV.map((n) => (
